...
class ShopifyClient {
	...
	public function getAuthorizeUrl($scope, $redirect_url='') {
		...
		if ($redirect_url != '')
		{
			$url .= "&redirect_uri=" . urlencode('https://shop.thefashionagent.co/partial_image/install.php');
		}
		return $url;
	}
	...
	private function curlParseHeaders($message_headers)
	{
		...
		$headers = [];
		$output = rtrim($message_headers);
		$data = explode("\n",$output);
		list(, $headers['http_status_code'], $headers['http_status_message']) = $data[0];
		// $headers['http_status_code'] = $data[0];
		array_shift($data);
